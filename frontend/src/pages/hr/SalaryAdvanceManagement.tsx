...
const SalaryAdvanceManagement: React.FC = () => {
  ...
  const hrRelevantAdvances = advances.filter(r => normalizeStatus(r.status) === 'forwardedtohr' || normalizeStatus(r.status) === 'approved' || normalizeStatus(r.status) === 'rejected');
  const hrPendingRequests = hrRelevantAdvances.filter(r => normalizeStatus(r.status) === 'forwardedtohr');
