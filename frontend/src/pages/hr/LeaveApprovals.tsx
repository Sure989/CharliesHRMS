...
const LeaveApprovals = () => {
          ...
          const mapped = requestsArray
            .map((r: any) => {
              return {
                ...
                managerId: r.managerId || r.employee?.managerId || r.branch?.managerId || '',
              };
            })
  ...
  // New workflow: HR only sees requests for employees with no branch or who are ops managers
  // Define helper to check if request is from ops manager or employee with no branch
  function isHRRelevantRequest(r: LeaveRequest) {
    // Defensive: branch may be empty string, null, or undefined
    const noBranch = !r.branch || r.branch.trim() === '';
    // Include if reviewed by an ops manager (opsManagerName exists and is not empty)
    const reviewedByOpsManager = r.opsManagerName && r.opsManagerName.trim() !== '';
    // Also include if employeeName matches common ops manager patterns
    const name = r.employeeName ? r.employeeName.toLowerCase() : '';
    const opsManagerPatterns = [
      'ops manager',
      'operations manager',
      'opsmgr',
      'ops mgr',
    ...
    const isOpsManagerByName = opsManagerPatterns.some(pattern => name.includes(pattern));
    // Include if employeeId matches opsManagerId
    const isOpsManagerById = r.employeeId && r.opsManagerId && String(r.employeeId) === String(r.opsManagerId);
    // Include if employeeId matches managerId
    const isManagerIdMatch = r.employeeId && r.managerId && String(r.employeeId) === String(r.managerId);
    return noBranch || reviewedByOpsManager || isOpsManagerByName || isOpsManagerById || isManagerIdMatch;
  }

  const hrPendingRequests = leaveRequests.filter(r => r.status === 'PENDING' && isHRRelevantRequest(r));
  const hrApprovedRequests = leaveRequests.filter(r => r.status === 'APPROVED' && isHRRelevantRequest(r));
  const hrRejectedRequests = leaveRequests.filter(r => r.status === 'REJECTED' && isHRRelevantRequest(r));

  // Only show relevant requests in the table
  const hrVisibleRequests = leaveRequests.filter(isHRRelevantRequest);
