...
const TeamsOverview = () => {
  // Helper to get branchId robustly
  // Best practice: Only use a unique branchId, never the branch name
  function getBranchId(user: any): string | undefined {
    if (user?.branchId) return user.branchId;
    if (user?.branch && typeof user.branch === 'object' && user.branch.id) return user.branch.id;
    // If branchId is missing, do not fallback to branch name
    return undefined;
  }
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    if (branchId) {
        ...
        branchService.getBranchEmployees(branchId)
          .then((members) => {
            ...
            const usersWithRoles = members
              ...
              // Exclude the branch manager (current user) from the team list
      ...
      setUsers([]);
      setLoading(false);
      setError('No branchId found for current user. Please contact your administrator or update your backend to include a unique branchId in the user profile.');
    }
  }, [user, user?.branchId, user?.branch]);
  ...
  const branchEmployees = useMemo(() => {
    ...
    // Exclude the branch manager (current user) from the team list
    return users.filter(
      (employee) => employee.branchId === branchId && employee.role === 'EMPLOYEE' && employee.id !== user?.id
    );
  }, [user, users]);
  ...
  const active = branchEmployees.filter(emp => (emp.status?.toLowerCase() === 'active')).length;
  const inactive = branchEmployees.filter(emp => (emp.status?.toLowerCase() === 'inactive')).length;
