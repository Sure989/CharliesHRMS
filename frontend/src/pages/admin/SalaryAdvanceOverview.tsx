  ...
  useEffect(() => {
    const loadSalaryAdvanceData = async () => {
      try {
        ...
        const response1 = await salaryAdvanceService.getSalaryAdvanceRequests({ status: 'hr' });
        const requests = response1.data || [];
        
        // In a real implementation, we would need to fetch from multiple endpoints
        // to get a complete view for the admin
        const response2 = await salaryAdvanceService.getSalaryAdvanceRequests({ status: 'operations' });
        const opsRequests = response2.data || [];
