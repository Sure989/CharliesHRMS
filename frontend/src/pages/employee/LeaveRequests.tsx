...
const LeaveRequests = () => {
  ...
  const loadRequests = useCallback(async () => {
    ...
    try {
      ...
      console.log('Raw leaveService.getLeaveRequests response:', response);
      ...
      if (Array.isArray(response)) {
        ...
        if (Array.isArray(response.data)) {
          ...
          Array.isArray((response.data as any).leaveRequests)
        ) {
          requestsArray = (response.data as any).leaveRequests;
        } else if (Array.isArray((response as any).leaveRequests)) {
          requestsArray = (response as any).leaveRequests;
          requestsArray = response.data;
        }
      }
      console.log('Requests array after improved parsing:', requestsArray);
      const mappedRequests = requestsArray.map((req: any) => ({
        ...
        branch: req.branch
          || (req.employee && req.employee.branch && req.employee.branch.name
        ...
        status: req.status ? req.status : 'UNKNOWN', // Show raw status for debugging
      }));
  ...
  // Match status color/icon logic to OpsManagerLeaveRequestPage
  ...
  const onSubmit = async (data: any) => {
    ...
    if (user?.role === 'OPERATIONS_MANAGER' || user?.position === 'Operations Manager') {
      toast({
        title: "Notice",
        description: "As an operations manager, leave approvals will be routed to HR.",
        variant: "default"
      });
    }
    try {
      ...
      // Use leaveService to submit and reload requests
      await leaveService.createLeaveRequest({
        employeeId: user?.employeeId,
        leaveTypeId: data.leaveType, // This should be the ID or code as expected by backend
      ...
      if (Array.isArray(response)) {
        ...
        requestsArray = (response as any).data;
      } else if (Array.isArray((response as any)?.requests)) {
        requestsArray = (response as any).requests;
      } else if (Array.isArray((response as any)?.leaveRequests)) {
        requestsArray = (response as any).leaveRequests;
      } else {
        requestsArray = [];
  ...
  // Use uppercase status for statistics, matching ops manager logic
      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          {stats.map((stat) => {
            ...
            else if (stat.title === 'Approved This Year') iconColor = "text-emerald-600";
            else if (stat.title === 'Days Taken') iconColor = "text-blue-600";
            else if (stat.title === 'Rejected') iconColor = "text-red-500";
            return (
              <Card key={stat.title}>
