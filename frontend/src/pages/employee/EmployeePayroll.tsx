...
const EmployeePayroll = () => {
  ...
  // Removed unused payrollRecords state
  ...
  const loadPayrollData = useCallback(async () => {
    ...
    try {
      setLoading(true);
      // If employeeId is a UUID, fetch employee record to get employeeNumber
      let effectiveUser = user;
      const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
      if (uuidRegex.test(user.employeeId)) {
        // Import employeeService dynamically to avoid circular deps
        const employeeServiceModule = await import('@/services/api/employee.service');
        const employeeService = employeeServiceModule.employeeService;
        const employeeRecord = await employeeService.getEmployeeById(user.employeeId);
        if (employeeRecord && employeeRecord.employeeId) {
          // Clone user and replace employeeId with business employee number
          effectiveUser = { ...user, employeeId: employeeRecord.employeeId };
        } else {
          setEmployeeInfo(null);
          setPayStubs([]);
          setLoading(false);
          toast({
            title: 'Error',
            description: 'Employee record not found for this user. Please contact support.',
            variant: 'destructive'
          });
          return;
        }
      }
      if (!effectiveUser.employeeId || typeof effectiveUser.employeeId !== 'string' || effectiveUser.employeeId === 'undefined') {
        setEmployeeInfo(null);
        setPayStubs([]);
        setLoading(false);
        console.error('Invalid businessEmployeeId for payroll API calls:', effectiveUser.employeeId);
        toast({
          title: 'Error',
          description: 'Invalid employee ID for payroll lookup. Please contact support.',
          variant: 'destructive'
        });
        return;
      }
      // Both getEmployeeInfo and generateEmployeePayStubs expect User objects with business employeeId
      const [employeeData, generatedPayStubs] = await Promise.all([
        PayrollDataService.getEmployeeInfo(effectiveUser),
        PayrollDataService.generateEmployeePayStubs(effectiveUser, selectedYear)
      ]);
      if (!employeeData) {
        setEmployeeInfo(null);
        setPayStubs([]);
        ...
        toast({
          title: 'Error',
          description: 'Employee not found. Please contact support.',
          variant: 'destructive'
        });
        return;
      }
              ...
              <CardContent className="p-0">
                <Table>
                  ...
                  <TableBody>
                    {payStubs.length === 0 ? (
                      <TableRow>
                        <TableCell colSpan={8} className="text-center text-muted-foreground">No pay stubs found for this year.</TableCell>
                      </TableRow>
                          ...
                          <div className="flex items-center space-x-2">
                            <Dialog>
                              ...
                              <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                                ...
                                {selectedPayStub && selectedPayStub.id === payStub.id && (
                                  <div className="space-y-6">
                ...
                <CardContent>
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="text-sm font-medium">{employeeInfo.accountType || '-'}</span>
                    </div>
                    <Button 
                      variant="outline" 
                      size="sm"
