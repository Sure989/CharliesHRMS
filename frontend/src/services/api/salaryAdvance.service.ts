...
export interface ApproveSalaryAdvanceRequest {
  requestId: string;
  approverId: string;
  status?: string; // Allow custom status (FORWARDEDTOHR or APPROVED)
...
class SalaryAdvanceService {
  ...
  async approveSalaryAdvanceRequest(approvalData: ApproveSalaryAdvanceRequest): Promise<SalaryAdvanceRequest> {
    try {
      const response = await apiClient.patch<SalaryAdvanceRequest>(`/salary-advances/${approvalData.requestId}`, {
  ...
  async rejectSalaryAdvanceRequest(rejectionData: RejectSalaryAdvanceRequest): Promise<SalaryAdvanceRequest> {
    try {
      const response = await apiClient.patch<SalaryAdvanceRequest>(`/salary-advances/${rejectionData.requestId}`, {
        status: 'REJECTED',
