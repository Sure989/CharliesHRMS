...
export class PayrollDataService {
  ...
  static async getEmployeeInfo(user: User | null) {
    ...
    try {
      ...
      // Use employeeNumber (business ID) for employeeService.getEmployeeByEmployeeId
      const employeeDetails = await employeeService.getEmployeeByEmployeeId(user.employeeNumber);
