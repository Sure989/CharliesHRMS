...
export interface User {
  ...
  employeeId?: string; // UUID for API requests
  employeeNumber?: string; // Business ID for display (EMP###)
  branchId?: string; // Only for employees working at specific venues (optional)
  branchName?: string; // Optional name of the branch for filtering
  ...
  branch?: Branch; // Computed from branchId for display
  department?: Department; // Computed from departmentId for display
}
...
export const ROLE_PERMISSIONS: { [key: string]: string[] } = {
  ...
  HR_MANAGER: [
    PERMISSIONS.HR_MANAGE_DEPARTMENTS, // Explicitly ensure HR_MANAGER can manage departments
