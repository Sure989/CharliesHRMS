...
router.delete('/:id', authenticate, restrictTo(['ADMIN']), validateUserId, validate, userController.deleteUser);
router.patch('/:id/status', validateUserId, validate, userController.updateUserStatus);
router.patch('/:id/permissions', authenticate, restrictTo(['ADMIN']), validateUserId, validate, userController.updateUserPermissions);
router.patch('/:id/role', authenticate, restrictTo(['ADMIN']), validateUserId, validate, userController.updateUserRole);
