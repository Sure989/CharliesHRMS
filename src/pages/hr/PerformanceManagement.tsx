...
const PerformanceManagement = () => {
  ...
  useEffect(() => {
    performanceReviewService.getPerformanceReviews().then((data: any[]) => {
      setReviews(data);
    });
  }, []);
  ...
  const handleAddReview = () => {
    ...
    const review: PerformanceReview = {
      ...
      branch: selectedEmployee.branch || 'Head Office',
      ...
      goals: newReview.goals.split(',').map(g => g.trim()).filter(g => g),
      achievements: [],
      areasForImprovement: [],
      feedback: newReview.feedback,
      reviewDate: newReview.reviewDate,
      reviewer: newReview.reviewer,
      reviewerRole: user?.role === 'ADMIN' ? 'Administrator' : 'HR Manager'
    };
    ...
    toast({
      ...
      description: `Review for ${review.employeeName} has been scheduled.`
    });
  };
