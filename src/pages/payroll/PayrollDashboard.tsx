    ...
    const fetchDashboardData = async () => {
      try {
        ...
        const [
          ...
          PayrollDataService.getPayrollStatus(),
          PayrollDataService.getPayrollTrends(),
          PayrollDataService.getStatutoryBreakdown(),
          PayrollDataService.getRecentActivity(4),
          PayrollDataService.getComplianceStatus()
        ]);
        
        setPayrollSummary(summary);
        setDepartmentBreakdown(deptBreakdown);
        setBranchBreakdown(branchBreak);
        setPayrollStatus(status);
        setPayrollTrendsData(trends);
        setStatutoryBreakdownData(statutory);
        ...
        toast({ 
          title: 'Error', 
          description: 'Failed to load payroll dashboard data. Some features may not be available.', 
          variant: 'destructive' 
        });
        ...
        setBranchBreakdown([]);
        setPayrollTrendsData([]);
        setStatutoryBreakdownData([]);
        setRecentActivity([]);
      } finally {
        setLoading(false);
      }
